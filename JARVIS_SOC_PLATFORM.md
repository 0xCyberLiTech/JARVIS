...
<!-- 0xCyberLiTech · v3.1 · 2026-05-15 — routing 4 branches · phi4:14b + qwen3:8b CR · mxbai-embed · 25 tests E2E Playwright + 705 tests pytest sur 33/34 modules avec coverage 39% lignes · ESLint 0 errors · MCP 10 outils · 32 modules Python (jarvis.py 4633L) · jarvis.css → 8 fichiers · refactor JS terminé : jarvis_main.js 7828→148L (−98,1%), 21 modules · git local + pre-commit hooks bloquants + hook pre-push pytest + ruff.toml · fix perf IPv6 (-97% latence interne) · circuit breaker Ollama (3 états + indicateur HUD) · score dette HONNÊTE ~92/100 (recalibré post-audit pytest --cov · +30 chantier dette 2026-05-14/15) -->
...
| **jarvis.py final** | **4633 L · 32 modules extraits · jarvis_main.js 148 L (−98,1%) · score dette HONNÊTE 92/100** (recalibré post-audit pytest --cov · +30 via chantier 2026-05-14/15 : Ruff + git + hooks + CSS 8 fichiers + audio_dsp.py + refactor JS terminé + 705 tests pytest sur 33/34 modules avec coverage 39% lignes + fix perf IPv6 + circuit breaker Ollama + hook pre-push) |
...
| ✅ | **Chantier dette technique 2026-05-15 (extension massive)** — score 78 → **92/100** (+14). **Refactor JS terminé** (`jarvis_main.js` 7828→**148 L** −98,1% cumul, 21 modules) · **705 tests pytest** sur 33/34 modules avec **coverage 39% lignes** (proxmox_api 93%, bypass_backup 96%, voice_lab 71%, deepfilter 84%, ssh_terminal 100%, stt 98%, rag_live 92%, soc.py 33%, jarvis.py 26% via Flask test_client) · **Phase 3 fix perf IPv6** (-97% latence interne via `OLLAMA_URL`/`JARVIS_BASE` → 127.0.0.1) · **Circuit breaker Ollama** (`ollama_circuit.py` 3 états + indicateur HUD `● OLLAMA`) · **hook pre-push pytest** · 3 bugs prod détectés+fixés (load-order, tts_cleaner, IPv6) · outil `tools/profile_perf.py` | 2026-05-15 |
