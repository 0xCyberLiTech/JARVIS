...
<!-- v2.1 — 2026-05-15 — Routing 4 branches + bypass · phi4:14b + qwen3:8b CR · mxbai-embed · NDT 100/100 (script auto) · score honnête global ~93/100 (départ réel 62, +30 chantier dette : refactor JS −85% cumul + 436 tests pytest sur 70% modules + fix perf IPv6 -97% latence interne) · 31 modules Python (jarvis.py 4633L) · jarvis.css → 8 fichiers · jarvis_main.js 7828→1181L (−85%), 19 modules JS · git local + pre-commit hooks bloquants + ruff.toml -->
...
Refactor JS exécuté 2026-05-14/15 : `jarvis_main.js` 7828→1181 L (−85% cumul), 13 modules extraits dans `static/js/` (16 modules total). 436 tests pytest ajoutés sur 23/33 modules Python (70%). Fix perf IPv6 (`OLLAMA_URL` + `JARVIS_BASE` → `127.0.0.1` explicite) : −97% latence sur clients internes (MCP, soc.py auto-engine). Score honnête global ~93/100.
