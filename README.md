...
## ⚡ Architecture Dual-IA — JARVIS orchestre Claude & Mistral

> *JARVIS ne travaille plus seul. Depuis juin 2026, il arbitre une chaîne de commandement entre deux intelligences : l'une qui pense, l'autre qui exécute.*

### La doctrine en une phrase

**Claude PILOTE** (lit, raisonne, commande) · **Mistral EXÉCUTANT** (obéit, génère, écrit) · **JARVIS ARBITRE** (verrous, journal, garde-fous).

Jamais les deux IA n'écrivent en même temps. Claude ne produit pas de code — il formule des ordres précis. Mistral ne décide rien — il exécute mot pour mot. JARVIS vérifie que personne ne sort de son rôle.

---

### Le tableau de commandement

<div align="center">
  <img src="Images/Jarvis-dualIA-commandant.png" alt="JARVIS Dual-IA — tableau de commandement : Claude aux commandes, Mistral exécutant, état nominal" width="900"/>
</div>

En tête du panneau **INFOGÉRANCE**, un bandeau vert indique **qui est aux commandes en temps réel** :

- **VERT — CLAUDE AUX COMMANDES · NOMINAL** : Claude pilote, lit le contexte, formule les ordres. Mistral attend.
- **ORANGE clignotant — MODE DÉGRADÉ · MISTRAL PILOTE** : Claude épuisé (fin de mois, quota dépassé). Mistral assume les deux rôles temporairement. JARVIS alerte vocalement à la bascule.

Les statistiques du mois s'affichent en direct : **ordres émis**, **tours de correction moyen**, **taux 1er tour OK** (cible ≥ 70 %), **coût d'exécution TTC**, **économie estimée** versus Claude seul sur le même volume.
...
<div align="center">
  <img src="Images/Jarvis-dualIA-journal.png" alt="JARVIS Dual-IA — journal de traçabilité : rôles, modèles, tokens, coût, tour de correction" width="900"/>
</div>

Chaque appel est tracé en temps réel : **qui a agi**, **quel modèle**, **combien de tokens**, **quel coût TTC**, **à quel tour de correction**.

Le code couleur ne laisse rien dans l'ombre :
- **Vert `◈ CLAUDE → ✦ MISTRAL (ORDRE)`** — ordre structuré Claude → Mistral via MCP (Phase 3).
- **Orange `✦ MISTRAL routing chat`** — message utilisateur routé vers Mistral en transparence.
- **Rouge `⚠ REPLI · MISTRAL PILOTE`** — mode dégradé actif, Claude temporairement absent.

Un tour > 1 s'affiche en rouge : signal immédiat qu'un ordre a nécessité une correction — et combien de fois.

---

### Les interrupteurs de commandement

<div align="center">
  <img src="Images/Jarvis-dualIA-switches.png" alt="JARVIS Dual-IA — interrupteurs kill-switch : Écrit, Vocal, Repli Claude → Mistral" width="900"/>
</div>

Trois interrupteurs physiques dans l'interface. **L'état nominal de tous = OFF** — JARVIS tourne 100 % local, 0 €, rien ne sort du réseau.

| Interrupteur | ON signifie | OFF signifie |
|---|---|---|
| **Écrit** | Mistral écrit à la place d'Ollama | Ollama local — gratuit, rien ne sort (défaut) |
| **Vocal** | Voix via Mistral streaming | Voix locale Kokoro/edge-tts — rapide, rien ne sort (défaut) |
| **Repli Claude → Mistral** | Claude absent — Mistral seul aux commandes (fin de mois) | Claude aux commandes (défaut) |

Chaque bascule déclenche une **confirmation explicite** avant exécution, et une **alerte vocale JARVIS** pour les transitions critiques (activation du repli, retour de Claude).

---

### La boucle complète — un exemple réel

Ce chantier lui-même a été validé en boucle réelle :

```
Claude lit le contexte (fichiers, doctrine, tests)
  ↓
Claude formule l'ORDRE structuré :
  ORDRE CLAUDE → MISTRAL
  TÂCHE : écrire fmt_duration(s: int) -> str
  CONTRAINTE : zéro import, zéro hardcodé
  RETOURNER : bloc Python uniquement

  ↓ jarvis_ia_exec → Mistral Codestral

Mistral génère le code exact demandé — 1er tour, 0 correction
  ↓
Claude valide
  ↓
JARVIS écrit le fichier + lance pytest (2102 tests)
  ↓
Résultat : vert. Rollback : non déclenché. Coût : 0.0003 € TTC.
Kill-switch OFF — retour état nominal.
```

**100 % de conformité au 1er tour. 0 régression. 3 garde-fous techniques actifs** (write_lock, header ORDRE obligatoire, audit de chaque tour). Le mode SOC reste, lui, 100 % local — Mistral n'y touche jamais.
