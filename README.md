...
Interface holographique, voix synthétique naturelle, écoute vocale continue,  
et intégration complète avec un **SOC de sécurité** homelab.

> Toutes les données restent sur votre machine. LLM, TTS et STT tournent localement.  
> Aucune connexion cloud requise pour le fonctionnement principal.
...
| **03** | TTS edge-tts, STT Whisper, DeepFilterNet NR | [03 — Pipeline Audio](./docs/03-PIPELINE-AUDIO.md) |
...
| `phi4-reasoning` | 12 Go | Analyse complexe |
...
- **Conversation** : streaming token par token, contexte persistant
- **Voix** : TTS naturel + STT continu + suppression bruit micro
- **SOC** : ban/unban IP, restart services, alertes vocales automatiques
- **Multi-modèles** : changement de modèle à chaud sans redémarrage
- **Interface** : thème holographique sombre, 10 onglets
