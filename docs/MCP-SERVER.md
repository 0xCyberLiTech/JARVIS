...
> **Split monolithe — Phase 3 (session 33) + chantier dette (2026-05-14)** : le serveur MCP `jarvis_mcp_server.py` n'a **PAS changé** — il consomme toujours les routes HTTP de `jarvis.py`. Les outils MCP fonctionnent identiquement après extraction de **31 modules** Python (Phase 3 : 30 modules audio/bypass/SSH/RAG/PVE/chat — voir [`ROUTING-JARVIS.md`](ROUTING-JARVIS.md) · + `audio_dsp.py` chaîne DSP, chantier 2026-05-14). `jarvis.py` est passé de **6592 → 4633 lignes**, les routes Flask consommées par MCP restent dans jarvis.py via wrappers DI vers les modules. Score honnête global : **78/100** (recalibré depuis 62 réel · +16 via chantier dette 2026-05-14 : git initialisé + pre-commit hooks bloquants + ruff.toml + CSS 8 fichiers + audio_dsp.py · NDT script auto 100/100 mais JS reste majoritairement monolithique, pas de CI cloud).
