...
| Tests | 22/25 | **1025 tests pytest · 0 skip · 0 fail** · 22 modules à 100% cov. Campagne couverture 2026-05-22 (étape 1 « couverture d'abord, refactor ensuite ») : +92 tests → `jarvis.py` 26→**38%**, `soc.py` 31→**44%**, coverage globale **58%**. −3 : `jarvis.py`/`soc.py` encore loin de la cible ~70-80% — campagne en cours (plusieurs sessions) avant tout refactor des monolithes. |
...
| Métrique | Valeur |
|---|---|
| **Tests pytest** | **1025 pass · 0 skip · 0 fail** (2026-05-22 : +92, campagne couverture étape 1) |
| **Coverage globale** | **58%** (6217 stmts · 2596 miss) |
...
| **Modules Python <50% cov** | `jarvis.py` 38% · `blueprints/soc.py` 44% (orchestrateurs Flask, complétés par 25 tests E2E · campagne couverture en cours) |
...
§0) + **campagne couverture étape 1** (+92 tests → `jarvis.py` 26→38%, `soc.py`
31→44%, total 58%) + correctif crash `/api/facts` sur corps non-dict → score
**88 → 91/100**. ⚠ Refactor des monolithes : décidé avec Marc = couverture
d'abord (cible ~70-80%), refactor ensuite — campagne en cours, plusieurs sessions.
...
| Tests pytest | **1025 pass · 0 skip · 0 fail** |
| Coverage globale | **58%** (6217 stmts) |
| Modules ≥100% cov | **22 modules** |
| ESLint | **0 erreur** (warnings camelCase cross-modules acceptés) |
| ruff | **0 erreur** |
| Pre-push hook | **pytest 1025 tests** bloquants |
...
*Document mis à jour le 2026-05-22 (audit dette complet honnête + correctifs + campagne couverture étape 1 + dé-duplication doc) — JARVIS 0xCyberLiTech v3.3 — 1025 tests pass · 0 skip · coverage 58% · 22 modules à 100% cov · score dette 91/100 honnête*
