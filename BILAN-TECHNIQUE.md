...
| Tests | 22/25 | **1043 tests pytest · 0 skip · 0 fail** · 22 modules à 100% cov. Campagne couverture 2026-05-22 (étape 1 « couverture d'abord, refactor ensuite ») : +110 tests → `jarvis.py` 26→**38%**, `soc.py` 31→**49%**, coverage globale **59%**. −3 : `jarvis.py`/`soc.py` encore sous la cible ~70-80% — campagne en cours (plusieurs sessions) avant tout refactor des monolithes. |
...
| Métrique | Valeur |
|---|---|
| **Tests pytest** | **1043 pass · 0 skip · 0 fail** (2026-05-22 : +110, campagne couverture étape 1) |
| **Coverage globale** | **59%** (6217 stmts · 2535 miss) |
...
| **Modules Python <50% cov** | `jarvis.py` 38% · `blueprints/soc.py` 49% (orchestrateurs Flask, complétés par 25 tests E2E · campagne couverture en cours) |
...
§0) + **campagne couverture étape 1** (+110 tests → `jarvis.py` 26→38%, `soc.py`
31→49%, total 59%) + correctif crash `/api/facts` sur corps non-dict → score
...
| Tests pytest | **1043 pass · 0 skip · 0 fail** |
| Coverage globale | **59%** (6217 stmts) |
| Modules ≥100% cov | **22 modules** |
| ESLint | **0 erreur** (warnings camelCase cross-modules acceptés) |
| ruff | **0 erreur** |
| Pre-push hook | **pytest 1043 tests** bloquants |
...
*Document mis à jour le 2026-05-22 (audit dette complet honnête + correctifs + campagne couverture étape 1 + dé-duplication doc) — JARVIS 0xCyberLiTech v3.3 — 1043 tests pass · 0 skip · coverage 59% · 22 modules à 100% cov · score dette 91/100 honnête*
