...
# soc_ip_deep / soc_suricata_ban / soc_threat_score / soc_reqhour : clusters
# extraits de soc.py (refactor incrémental). security_whitelists : source unique
# de la politique de whitelist de sécurité.
import soc_ip_deep
import soc_reqhour
...
# Cluster pic de trafic req/h (_reqhour_candidates + _reqhour_inject_suricata +
# _soc_reqhour_check) extrait dans soc_reqhour.py — refactor incrémental étape 4
# (2026-05-22). DI plus large que les étapes 1-3 (cluster couplé au cœur ban) :
# _speak et le dict _SOC_AUTO_BANNED, réassignés après le chargement du module,
# sont injectés via des lambdas résolues à l'appel. Alias légers conservés → la
# route force-autoban et _soc_monitor_loop restent inchangés.
soc_reqhour.init(
    is_whitelisted=_is_whitelisted,
    auto_banned_ts=lambda ip: _SOC_AUTO_BANNED.get(ip, 0),
    soc_cooldown_ok=_soc_cooldown_ok,
    ip_try_mark_banned=_ip_try_mark_banned,
    save_auto_banned=_save_auto_banned,
    ban_ip_ssh=_ban_ip_ssh,
    soc_log=_soc_log,
    speak=lambda *a, **k: _speak(*a, **k),
    stage_priority=_STAGE_PRIORITY,
    cooldown_ip=_SOC_COOLDOWN_IP,
    req_hour_seuil=_SOC_REQ_HOUR_SEUIL,
    autoban_min_hits=_AUTOBAN_MIN_HITS,
)
_reqhour_candidates      = soc_reqhour._reqhour_candidates
_reqhour_inject_suricata = soc_reqhour._reqhour_inject_suricata
_soc_reqhour_check       = soc_reqhour._soc_reqhour_check
...
# Cluster scoring menace (_threat_score_from_json + _check_threat_level +
# _check_escalation) extrait dans soc_threat_score.py — refactor incrémental
# étape 3 (2026-05-22). DI : _soc_cooldown_ok + _ip_to_tts injectés ; alias
# légers conservés pour que les appelants (route + _soc_monitor_loop) restent
# inchangés.
soc_threat_score.init(_soc_cooldown_ok, _ip_to_tts)
_threat_score_from_json = soc_threat_score._threat_score_from_json
_check_threat_level     = soc_threat_score._check_threat_level
_check_escalation       = soc_threat_score._check_escalation
...
def _soc_monitor_loop():
    ...
    while not _soc_stop_evt.is_set():
        ...
        try:
            ...
            # KC v4 Sprint 7 (2026-05-18) : annonce dédiée escalation (badge 🔥 backend)
            # Cooldown court interne (10 min) pour signaler les IPs en progression
            # rapide même si threat_level non déclenché.
            try:
                escal_parts = _check_escalation(ts)
                if escal_parts:
                    _speak(" ".join(escal_parts))
            except Exception as _ee:
                _log.error(f"[JARVIS-SOC-ESCAL] {_ee}")
