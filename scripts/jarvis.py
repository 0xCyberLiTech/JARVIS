...
- Infrastructure connue : Proxmox=192.168.1.20, srv-ngix=192.168.1.50, clt=192.168.1.12, pa85=192.168.1.13, srv-dev-1=192.168.1.21, JARVIS=192.168.1.107, Freebox=192.168.1.254
...
# ── numpy (toujours disponible, indépendant du DSP) ──────────
try:
    import numpy as np
    _NP_AVAILABLE = True
except ImportError:
...
# ── Moteur DSP (scipy + numpy + miniaudio) ───────────────────
try:
...
# ── DeepFilterNet — module dédié `deepfilter.py` (Phase 3 module 4) ───────────
import deepfilter as _df

# ── FX RACK multi-effets CUDA ─────────────────────────────────────────
_FX_IR_CACHE = {}
...
def _apply_fx_rack(signal: 'np.ndarray', sr: int) -> 'np.ndarray':
