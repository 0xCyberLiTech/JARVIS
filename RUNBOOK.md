...
- ⚠ **Tests Python** : `python -m pytest` (799 tests sur 34/34 modules · coverage 39% lignes via `pytest --cov=scripts` · pyproject.toml `[tool.pytest.ini_options]` · `tests/python/conftest.py` ajoute `scripts/` au sys.path). Doit être vert avant tout commit Python ET avant tout push (hook pre-push installé).
- ⚠ **Hook pre-push pytest** : `pre-commit install --hook-type pre-push` lance les 799 tests avant chaque `git push`. Bloque sur tests rouges. Alternative locale à CI cloud (impossible « rien sur le web »). Bypass : `git push --no-verify` (à éviter — c'est précisément contre ça).
