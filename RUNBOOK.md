...
| Élément | Emplacement |
|---|---|
| Serveur Flask | `JARVIS/scripts/jarvis.py` (~4739 L + 33 modules Python) |
...
- ⚠ **Tests Python** : `python -m pytest` (**933 tests** sur 32 modules · 22 à 100% cov · coverage RÉELLE **51%** via `pytest --cov=scripts` · pyproject.toml `[tool.pytest.ini_options]` · `tests/python/conftest.py` ajoute `scripts/` au sys.path). Doit être vert avant tout commit Python ET avant tout push (hook pre-push installé).
- ⚠ **Hook pre-push pytest** : `pre-commit install --hook-type pre-push` lance les 933 tests avant chaque `git push`. Bloque sur tests rouges. Alternative locale à CI cloud (impossible « rien sur le web »). Bypass : `git push --no-verify` (à éviter — c'est précisément contre ça).
