...
- ⚠ **Tests Python** : `python -m pytest` (705 tests sur 33/34 modules · coverage 39% lignes via `pytest --cov=scripts` · pyproject.toml `[tool.pytest.ini_options]` · `tests/python/conftest.py` ajoute `scripts/` au sys.path). Doit être vert avant tout commit Python ET avant tout push (hook pre-push installé).
- ⚠ **Hook pre-push pytest** : `pre-commit install --hook-type pre-push` lance les 705 tests avant chaque `git push`. Bloque sur tests rouges. Alternative locale à CI cloud (impossible « rien sur le web »). Bypass : `git push --no-verify` (à éviter — c'est précisément contre ça).
- ⚠ **Circuit breaker Ollama** (`scripts/ollama_circuit.py`) : si Ollama tombe (3 erreurs consécutives) → circuit OPEN, refus immédiat des requêtes Ollama (1 ms vs timeout 30 s). Auto-recovery via test HALF_OPEN après 30 s, backoff exponentiel ×2 (max 5 min). Indicateur visuel HUD `● OLLAMA` (vert/orange/rouge). Endpoint `/api/ollama-status` retourne `{running, state, retry_in_s, current_timeout_s}`.
